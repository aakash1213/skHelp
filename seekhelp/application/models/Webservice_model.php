...
class Webservice_model extends CI_Model {
	...
	/* Check user is registered with the mobile number is exist or not */
	function is_number_exist ( $email, $user_id = '') {
		$this->db->select ( 'user_id, name' );
		if ($user_id != '')
			$this->db->where ( 'user_id != ', $user_id );
	...
	/* Get User data by user id from the db */
	function getUserData($ur_id) {
		$this->db->select ( '*' );
		if ($ur_id != ''){
